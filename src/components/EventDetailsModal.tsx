...
export const EventDetailsModal: React.FC<EventDetailsModalProps> = ({
  ...
  const getVisibilityInfo = (visibility: string) => {
    const visibilityMap = {
      ...
      private: { label: "Private", color: "text-yellow-600", bg: "bg-yellow-100" },
      archived: { label: "Archived", color: "text-gray-600", bg: "bg-gray-100" },
    };
    return visibilityMap[visibility as keyof typeof visibilityMap] || visibilityMap.public;
  };

  const getCategoryInfo = (category: string) => {
    const categoryMap = {
      business: { label: "Business", color: "text-blue-600", bg: "bg-blue-100" },
      technology: { label: "Technology", color: "text-purple-600", bg: "bg-purple-100" },
      ...
      activism: { label: "Activism", color: "text-indigo-600", bg: "bg-indigo-100" },
        ...
        >
          ...
          >
            ...
            <div className="p-6 space-y-6">
              {/* Event Title */}
              <div>
                <h3 className="text-3xl font-bold text-primary mb-2">{event.title}</h3>
                <div className="flex flex-wrap gap-2">
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${categoryInfo.bg} ${categoryInfo.color}`}>
                    <Tag className="w-4 h-4 mr-1" />
                    {categoryInfo.label}
                  </span>
                  <span className={`inline-flex items-center px-3 py-1 rounded-full text-sm font-medium ${visibilityInfo.bg} ${visibilityInfo.color}`}>
              ...
              <div className="bg-blue-50 rounded-lg p-4">
                <h4 className="font-semibold text-primary mb-2">Template Information</h4>
                <div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
                    <span className="text-gray-600">Image Placeholders:</span>
                    <span className="ml-2 font-medium">{event.image_placeholders.length}</span>
                  </div>
                  <div>
                    <span className="text-gray-600">Text Placeholders:</span>
                    <span className="ml-2 font-medium">{event.text_placeholders.length}</span>
                  </div>
                </div>
              </div>
